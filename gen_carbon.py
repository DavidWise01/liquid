...
def portrait_liquid(member):
    """Hydor, the liquid elemental: a serene figure whose face is a liquid-metal
    mirror — smooth, featureless, ever-rewriting (the memristive substrate) — with
    flowing water-hair, drifting ion droplets, and a forming teardrop. Wet register."""
    import math
    g=[VOID]*(LW*LH); drawn=[False]*(LW*LH)
    def put(x,y,c):
    ...
    def ell(cx,cy,rx,ry,c):
        for y in range(int(cy-ry),int(cy+ry)+1):
            for x in range(int(cx-rx),int(cx+rx)+1):
                if ((x-cx)/rx)**2+((y-cy)/ry)**2<=1.0: put(x,y,c)
    rb=hashlib.sha256(("liquid:"+member["name"]).encode()).digest()
    CYAN=(56,214,230); TEAL=(47,194,166); BLUE=(70,110,210); SILVER=(196,208,220)
    SILV_HI=(236,244,250); SILV_SH=(120,140,158); AMBER=(255,178,74)
    cx=32; hy=33
    # faint cyan top glow
    for y in range(LH):
        for x in range(LW):
            d=((x-cx)**2/640.0+(y-8)**2/300.0)
            if d<1: soft(x,y, CYAN, 0.10*(1-d))
    # drifting ion droplets (behind)
    drops=[(13,16,4),(51,12,5),(52,33,3),(12,40,3),(50,55,4),(20,10,2)]
    for k,(sx,sy,sr) in enumerate(drops):
        sx+=(rb[k]%3)-1; sy+=(rb[k+6]%3)-1
        col = AMBER if k%3==0 else mix(CYAN,BLUE,0.4)
        ell(sx,sy,sr,sr-1, col); ell(sx-1,sy-1,max(1,sr-2),max(1,sr-2), tint(col,0.3))
    # robe — water (teal/blue), shoulders
    robe=(38,86,118); robe_sh=shade(robe,0.32)
    rect(8,58,55,79, robe)
    for x in range(8,56):
        if x<13 or x>50: rect(x,58,x,60,VOID)
    rect(20,57,44,60, robe_sh)
    for k in range(7): put(32-k,57+k, robe_sh); put(32+k,57+k, robe_sh)
    # wavy flow lines on the robe
    for fy in (64,70,76):
        for x in range(12,53):
            yy=fy+round(1.6*math.sin((x+fy)*0.5))
            soft(x,yy, mix(robe,CYAN,0.5), 0.4)
    # neck
    rect(28,47,36,57, shade(SILVER,0.3))
    # flowing water-hair behind/around the head (cyan/teal cascades)
    for side in (-1,1):
        for s in range(5):
            bx=cx+side*(11+s*1.5)
            for y in range(16,56):
                xx=bx+side*round(3*math.sin((y*0.32)+s)) + round(2*math.sin(y*0.18))
                soft(xx,y, mix(TEAL,CYAN,(s%3)/3), 0.5)
                if s<2: put(xx,y, mix(TEAL,CYAN,0.5))
    # the liquid-metal mirror face
    ell(cx,hy,13,17, SILVER)
    # vertical sheen gradient + a bright diagonal reflection streak
    for y in range(hy-17,hy+18):
        for x in range(cx-13,cx+14):
            i=y*LW+x
            if 0<=i<LW*LH and drawn[i] and g[i]==SILVER:
                t=(x-(cx-13))/26.0
                g[i]=mix(SILV_SH, SILV_HI, t)            # left dark -> right light
    for d in range(-6,7):                                # diagonal highlight
        put(cx-4+d, hy-8+d, SILV_HI); put(cx-3+d, hy-8+d, tint(SILV_HI,0.3))
    # two ripple lines across the mirror (no fixed features — it rewrites)
    for ry in (hy-4, hy+6):
        for x in range(cx-10,cx+11):
            yy=ry+round(1.4*math.sin((x)*0.5))
            if 0<=yy<LH and drawn[yy*LW+x]: soft(x,yy, SILV_SH, 0.45)
    # a single forming teardrop at the chin
    put(cx,hy+19,CYAN); put(cx,hy+21,mix(CYAN,SILV_HI,0.4)); put(cx,hy+23,CYAN)
    ell(cx,hy+25,2,3, CYAN)
    # a couple amber ions caught in the hair
    put(cx-15,hy-2,AMBER); put(cx+15,hy+4,AMBER); put(cx-13,hy+14,AMBER)
    return finish(g, drawn)


PORTRAITS = {"gravity": portrait_aether, "lattice": portrait_leech, "liquid": portrait_liquid}
